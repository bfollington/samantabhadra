...
export const semanticSearchMemos = tool({
  ...
  parameters: z.object({
    ...
    includeExactMatches: z.boolean().optional().describe("Whether to include exact text matches in results (default: true)"),
  }),
  execute: async ({ query, limit = 5, includeExactMatches = true }) => {
    ...
    try {
          ...
          if (words.length > 0) {
            ...
            for (const word of significantWords) {
              ...
              try {
                ...
                console.error(`Error in word match search for '${word}':`, wordError);
        ...
        try {
          ...
          if (vectorResults?.matches?.length > 0) {
            ...
            for (const match of vectorResults.matches) {
              if (match?.metadata?.memo_id && typeof match.metadata.memo_id === 'string') {
          ...
          if (embeddingError instanceof Error && embeddingError.message.includes('Authentication')) {
            console.error('Authentication error detected - please check your Cloudflare credentials');
          }
        }
      } catch (error) {
      ...
      try {
        ...
        const searchWords = query.split(/\s+/).filter(w => w.length > 2).slice(0, 2);
        ...
        if (searchWords.length > 0) {
          ...
          for (const word of searchWords) {
            ...
            try {
              ...
              console.error(`Error in fallback word search for '${word}':`, wordError);
              // Continue with other words
            }
          ...
          if (keywordResults.length > 0) {
            console.log(`Found ${keywordResults.length} keyword matches as fallback`);
